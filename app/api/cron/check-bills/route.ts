  ...
  try {
    ...
    const accountsDue = accounts.filter((account) => {
      ...
      const hoursSinceCheck =
        (now.getTime() - new Date(account.lastCheckedAt).getTime()) / (1000 * 60 * 60)
        ...
        if (
          ...
          if (needsNotification) {
            ...
            try {
              ...
              console.error(`[Cron] Email failed for account ${account.id}:`, emailErr)
              summary.errors++
            }
          }
        }
    ...
    await Promise.all(proAccounts.map((a) => proLimit(() => processAccount(a))))
    await Promise.all(freeAccounts.map((a) => freeLimit(() => processAccount(a))))
